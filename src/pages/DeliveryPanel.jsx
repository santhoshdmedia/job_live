...
const CollectPaymentModal = ({ job, onClose, onCollected }) => {
  ...
  // Calculate updated balance after discount
  const updatedBalance = originalBalance - (discountApplied ? discountAmount : 0);
  ...
  const handleSave = async () => {
    ...
    if (amt > updatedBalance)
      return show(
        `Amount exceeds discounted balance of ₹${updatedBalance.toLocaleString("en-IN")}`,
        "warning",
      );
    ...
    try {
      ...
      await api(`/jobs/${job._id}`, {
        ...
        body: JSON.stringify({
          ...
          payment_mode: mode,
          ...(discountApplied && discountAmount > 0 && { discount_amount: discountAmount }),
        }),
      });
      
      onCollected(newPaid, mode);
      onClose();
      show(`Payment of ₹${amt.toLocaleString("en-IN")} collected successfully!`, "success");
    } catch (err) {
      ...
      <div className="bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl border border-gray-100 shadow-2xl">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="bg-green-50 border border-green-100 rounded-xl p-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (disc > 0 && disc <= originalBalance) {
            ...
            {!discountApplied && (
              <div className="text-xs text-gray-400 mt-2">
                Enter a discount amount to reduce the customer's balance
              </div>
            )}
          </div>
          ...
          <div className={`rounded-xl p-3 text-center border-2 transition-all ${
            ...
            <div className={`text-xs font-semibold mb-1 ${discountApplied && discountAmount > 0 ? 'text-blue-600' : 'text-gray-500'}`}>
              {discountApplied && discountAmount > 0 ? "💰 Balance after discount" : "Current Balance"}
            </div>
            <div className={`text-2xl font-bold mt-1 ${discountApplied && discountAmount > 0 ? 'text-blue-700' : 'text-gray-700'}`}>
              ₹{updatedBalance.toLocaleString("en-IN")}
            </div>
            {discountApplied && discountAmount > 0 && (
              <div className="text-xs text-green-600 mt-1">
                Customer saves: ₹{discountAmount.toLocaleString("en-IN")}
              </div>
            )}
          </div>
          ...
          <Field label="Amount collecting" required>
            <Inp
              type="number"
              placeholder={`Max ₹${updatedBalance}`}
              value={amount}
              ...
              step={1}
            />
            {discountApplied && updatedBalance > 0 && amount && (
              <div className="text-xs text-gray-500 mt-1">
                Remaining after payment: ₹{(updatedBalance - parseFloat(amount || 0)).toLocaleString("en-IN")}
              </div>
            )}
          </Field>
          ...
          {(discountApplied || amount) && (
            <div className="bg-gray-50 rounded-xl p-3 space-y-1.5 text-sm">
              ...
              <div className="flex justify-between pt-1 border-t border-gray-200">
                ...
                <span className="font-bold text-blue-600">₹{updatedBalance.toLocaleString("en-IN")}</span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-1 pb-4">
            ...
            <Btn 
              ...
              loading={saving} 
              onClick={handleSave}
              disabled={!amount || parseFloat(amount) <= 0 || parseFloat(amount) > updatedBalance}
            >
              Save payment
            </Btn>
          </div>
