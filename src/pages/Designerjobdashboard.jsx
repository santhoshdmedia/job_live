...
const CustomerInfoBlock = ({
  ...
  // Designer — show masked info + request button
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600 }}>Created by</div>
      ...
      <div style={{
        ...
        background: "#f1f5f9", borderRadius: 8, padding: "8px 10px",
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "#b0bec5", letterSpacing: "0.12em" }}>
            +91 ●●●●●●●●●●
          </div>
        </div>
        <div style={{ flexShrink: 0 }}>
          {requestStatus === "none" || requestStatus === "rejected" ? (
            ...
            <span style={{
              ...
              <HourglassOutlined style={{ animation: "spin 2s linear infinite" }} />
              Pending…
            </span>
          ) : null}
        </div>
      ...
      {requestStatus === "rejected" && (
        <div style={{ marginTop: 6, fontSize: 10, color: "#ef4444", fontWeight: 600 }}>
          ...
          Previous request was denied. You may submit a new request.
        </div>
      )}
    </div>
  );
  ...
  const handleDownload = async () => {
    try {
      ...
      const a    = Object.assign(document.createElement("a"), {
        href: url, download: `design_${Date.now()}.${ext}`,
      });
    ...
    <div style={{
      ...
      {isImage ? (
        <div style={{ padding: 10, textAlign: "center", background: isSample ? "#eef0ff" : "#f5f3ff" }}>
          <img src={fileUrl} alt="Design preview" style={{
            maxWidth: "100%", maxHeight: 220, objectFit: "contain",
            borderRadius: 6, boxShadow: "0 2px 8px rgba(124,58,237,0.15)",
          }} />
          {isSample && (
        ...
        <div style={{ padding: "10px 12px", fontSize: 12, color: "#6b7280" }}>
          📄 PDF — <a href={fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#7c3aed" }}>Click to view</a>
        </div>
      ) : (
        <div style={{ padding: "10px 12px", fontSize: 12, color: "#6b7280" }}>
          📎 File — <a href={fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#7c3aed" }}>Download / View</a>
        </div>
      )}
    </div>
  );
      ...
      <div style={{ padding: "12px 14px", background: "#fffbeb" }}>
        <div style={{
          ...
          <div style={{ fontSize: 11, color: "#991b1b", lineHeight: 1.5 }}>
            <strong>Auto-cleanup in 48 hours.</strong> Files in the shared Drive folder are deleted after 48 hours. Download or move originals before then.
          </div>
        </div>
        ...
        <div style={{ marginTop: 10, fontSize: 10, color: "#92400e", textAlign: "center", fontStyle: "italic" }}>
          ⏱ Drive files auto-deleted 48 hrs after upload.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div
      style={{
        ...
        padding: "14px", background: "#faf5ff",
    ...
    >
      ...
      {compressing ? (
        <div style={{ color: "#7c3aed", fontSize: 12 }}>
          <Spin size="small" style={{ marginRight: 8 }} />Compressing…
        </div>
      ) : sampleInfo ? (
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 4 }}>
            Images auto-compressed to ≤1 MB · PDF accepted as-is
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        <CustomerInfoBlock
          job={job}
          requestStatus={infoRequestStatus}
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          {!isSuperAdmin && (
            <span style={{ marginLeft: 6 }}>
              <InfoRequestBadge requestStatus={infoRequestStatus} />
            </span>
          )}
        </div>
          ...
          {(job.cart_items || []).slice(0, 2).map((item, i) => (
            <div key={i} style={{
              ...
              <span style={{ fontWeight: 600 }}>
                {item.product_name} (
                {item.variation} | {item.printing_type})
        ...
        {sessionData && (sessionData.total_duration_seconds > 0 || sessionData.worked_days > 0) && (
          <div style={{
            ...
            <span style={{ color: "#374151" }}>
              {sessionData.worked_days} day{sessionData.worked_days !== 1 ? "s" : ""} · {sessionData.closed_sessions} session{sessionData.closed_sessions !== 1 ? "s" : ""}
            </span>
          </div>
        )}
        ...
        {hasDesign && (
          <div style={{
            ...
            marginBottom: 10, background: "#faf5ff",
...
const DesignerJobDashboard = () => {
  ...
  // Info request state: { [jobId]: { status, has_access, request } }
  const [infoRequestMap,  setInfoRequestMap]  = useState({});
  const [requestingInfo,  setRequestingInfo]  = useState(null); // jobId currently requesting
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      ...
      // Fetch info request statuses (only for non-admin designers)
      if (!isSuperAdmin) {
        const reqMap = {};
        await Promise.all(
          myJobs.map(async (j) => {
  ...
  const startLiveTimer = (jobId, sessionStartedAt, previousTotalSecs = 0) => {
    ...
    timerRefs.current[jobId] = setInterval(() => {
      setLiveTimers((prev) => {
        ...
        return {
          ...prev,
          [jobId]: { ...entry, currentSessionSecs: Math.floor((Date.now() - new Date(entry.startedAt).getTime()) / 1000) },
        };
      });
    }, 1000);
  };
    ...
    try {
      const res  = await fetch(INFO_BASE, {
        ...
        body: JSON.stringify({
          ...
          requested_by: { user_id: userId, name: userName, role: user.role || "designing team" },
      ...
      setInfoRequestMap((prev) => ({
        ...prev,
        [requestJob._id]: { status: "pending", has_access: false, request: data.data },
      }));
    } catch (err) {
  ...
  const pendingInfoCount   = Object.values(infoRequestMap).filter((r) => r?.status === "pending").length;
    ...
    <div style={{
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>
              ...
              <span style={{ color: "#16a34a", fontWeight: 600 }}>
                {isSuperAdmin ? "All design jobs" : `Assigned to ${userName}`}
              </span>
            </p>
          </div>
        </div>
      ...
      <div style={{
        ...
        {[
          ...
          ...(!isSuperAdmin ? [
            { label: "Access Pending", value: Object.values(infoRequestMap).filter((r) => r?.status === "pending").length,   color: "#d97706", bg: "#fffbeb" },
          ] : []),
            ...
            {jobs.map((job) => {
              ...
              return (
                <JobCard
                  ...
                  infoRequestStatus={reqInfo.status}
                  hasInfoAccess={reqInfo.has_access}
                  isSuperAdmin={isSuperAdmin}
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            key="ok" type="primary" loading={submittingReq}
        ...
        <div>
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#0c4a6e", lineHeight: 1.6 }}>
              Customer contact details are protected to safeguard privacy.
              Your request will be reviewed by an admin. Once approved, you'll be able to see
              the customer's name and phone number for this job.
            </p>
          </div>
          ...
          {requestJob && (
            <div style={{
              ...
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>
                {(requestJob.cart_items || []).map((i) => i.product_name).join(", ")}
              </div>
            </div>
          )}
          ...
          <TextArea
            rows={3}
            placeholder="e.g. Need to coordinate delivery timing with customer…"
            value={requestReason}
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: "#6b7280" }}>
            ...
            An admin will review your request. You'll see the status update on the job card.
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            key="ok" type="primary" loading={actioning}
            onClick={handleConfirmAction}
            style={{
              background:
                pendingAction?.action === "on_hold" ? "#f97316"
                : pendingAction?.action === "completed" ? "#16a34a"
                : "#1e40af",
              border: "none",
            }}
          >
          ...
          return (
            <div>
              <div style={{
                ...
                padding: "10px 12px", marginBottom: 14, border: "1px solid #e5e7eb",
              }}>
                ...
                <div style={{
                  ...
                  {sessData.worked_days > 0 && (
                    <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
                      {sessData.worked_days} day{sessData.worked_days !== 1 ? "s" : ""} worked · {sessData.closed_sessions} session{sessData.closed_sessions !== 1 ? "s" : ""} closed
                    </div>
                  )}
                    ...
                    <div style={{ fontSize: 12, color: "#374151", marginTop: 4 }}>
                      ...
                      {designJob.customer_phone && (
                        <span style={{ marginLeft: 8, color: "#6b7280" }}>
                          <PhoneOutlined style={{ marginRight: 3 }} />{designJob.customer_phone}
                        </span>
                      )}
                    </div>
                  )}
              ...
              <div style={{ marginTop: 10 }}>
                {(designJob.cart_items || []).map((item, i) => (
                  <div key={i} style={{
                    ...
                    display: "flex", justifyContent: "space-between",
                    padding: "3px 0", borderTop: i > 0 ? "1px solid #e9d5ff" : undefined,
                  }}>
                    <span>
            ...
            <div style={{ marginBottom: 6 }}>
              ...
              <Button
                ...
                style={{
                  ...
                  background: "#7c3aed", border: "none", borderRadius: 8, fontWeight: 600,
                }}
              >
