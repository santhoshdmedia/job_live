...
const DesignFilePreview = ({
  ...
  return (
    <div
      style={{
        border: `1px solid ${isSample ? "#a5b4fc" : "#c4b5fd"}`,
        borderRadius: 10,
        overflow: "hidden",
        marginBottom: 14,
      ...
      {isImage ? (
        <div
          style={{
            padding: 10,
            textAlign: "center",
            background: isSample ? "#eef0ff" : "#f5f3ff",
          }}
        >
          <img
            src={fileUrl}
      ...
      <div style={{ padding: "12px 14px", background: "#fffbeb" }}>
        ...
        >
          ...
          <div style={{ fontSize: 11, color: "#991b1b", lineHeight: 1.5 }}>
            <strong>Auto-cleanup in 48 hours.</strong> Files uploaded to the
            shared Drive folder are automatically deleted after 48 hours to keep
            storage clean. Download or move the original to permanent storage
            before then.
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          {[
            {
              n: "1",
              text: "Click the button below to open the shared Drive folder",
            },
            {
              n: "2",
              text: `Upload your original high-quality design file for job ${jobNo}`,
            },
            {
              n: "3",
              text: 'Right-click the file → "Get link" → copy and paste it below',
            },
          ].map(({ n, text }) => (
            <div
              key={n}
        ...
        {driveLinkValue && (
          ...
          >
            <CheckCircleOutlined /> Drive link recorded — will be saved with the
            job.
          </div>
        )}
        ...
        >
          ⏱ Drive files auto-deleted 48 hrs after upload — save originals
          elsewhere promptly.
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      {compressing ? (
        ...
        <div>
          <UploadOutlined
            style={{ fontSize: 22, color: "#7c3aed", marginBottom: 6 }}
          />
...
const JobCard = ({
  ...
  const hasDesign = !!(
    job.design_file || job.cart_items?.some((i) => i.design_file)
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        >
          ...
          {(job.cart_items || []).length > 2 && (
            <div style={{ fontSize: 11, color: "#6b7280" }}>
              +{job.cart_items.length - 2} more items
            </div>
          )}
        </div>

        {sessionData &&
          (sessionData.total_duration_seconds > 0 ||
            sessionData.worked_days > 0) && (
            <div
              style={{
                display: "flex",
                justifyContent: "space-between",
            ...
            >
              ...
              <span style={{ color: "#374151" }}>
                {sessionData.worked_days} day
                {sessionData.worked_days !== 1 ? "s" : ""}·{" "}
                {sessionData.closed_sessions} session
                {sessionData.closed_sessions !== 1 ? "s" : ""}
              </span>
            </div>
          )}
        ...
        <div style={{ marginBottom: 12 }}>
          <div
            style={{ fontSize: 11, color: isOverdue ? "#ef4444" : "#6b7280" }}
          >
...
const DesignerJobDashboard = () => {
  ...
  const [designDriveLink, setDesignDriveLink] = useState(""); // path from UploadHelper (sample)
  ...
  const loadJobs = useCallback(async () => {
    const isSuperAdmin = user.role === "super_admin"; // adjust key to match your profile shape
    if (!userId) return;
    setLoading(true);
    try {
      let myJobs = [];
      ...
      if (isSuperAdmin) {
        // Fetch ALL jobs and filter for design stage
        ...
        // Regular designer — only their assigned jobs
        const res = await fetch(`${BASE}/assigned-to/${userId}`, {
          headers: authHeader(),
        });
        const data = await res.json();
  ...
  const startLiveTimer = (jobId, sessionStartedAt, previousTotalSecs = 0) => {
    ...
    timerRefs.current[jobId] = setInterval(() => {
      setLiveTimers((prev) => {
        ...
        return {
          ...prev,
          [jobId]: { ...entry, currentSessionSecs: sessionSecs },
        };
      });
    }, 1000);
  };
  ...
  const handleUploadDesign = async () => {
    ...
    try {
      ...
      message.success(
        "Design saved! Sample stored; original available via Drive link.",
      );
  ...
  const cartDesignFile = designJob?.cart_items?.find(
    (i) => i.design_file,
  )?.design_file;
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>
              Hi <strong>{userName}</strong> · {jobs.length} active job
              {jobs.length !== 1 ? "s" : ""}· Updated{" "}
              {lastRefresh.format("HH:mm:ss")}·{" "}
              <span style={{ color: "#16a34a", fontWeight: 600 }}>
                {user.role === "super_admin"
                  ? "All design jobs"
                  : `jobs assigned to ${user.name || user.fullName || user.username || "you"}`}
              </span>
            </p>
          </div>
        </div>
      ...
      >
        {[
          ...
          {
            label: "On Hold",
            value: jobs.filter((j) => j.job_status === "on_hold").length,
            color: "#f97316",
            bg: "#fff7ed",
          },
          {
            ...
            value: jobs.filter(
              (j) => j.design_file || j.cart_items?.some((i) => i.design_file),
            ).length,
            color: "#8b5cf6",
            bg: "#f5f3ff",
          },
          ...
          >
            {jobs.map((job) => {
              ...
              const displaySess = sessData
                ? {
                    ...sessData,
                    total_duration_seconds: getLiveDisplaySecs(job._id),
                  }
                : null;
              return (
                <JobCard
              ...
              <div>
                ...
                >
                  <div style={{ fontWeight: 700, color: "#111827" }}>
                    {pendingAction.job.customer_name}
                  </div>
                  <div style={{ fontSize: 12, color: "#6b7280" }}>
                    Stage:{" "}
                    <strong>
                      {pendingAction.job.current_stage?.stage_label || "Design"}
                    </strong>
                  </div>
                </div>
                  ...
                  sessData && (
                    ...
                    >
                      ...
                      {sessData.worked_days > 0 && (
                        ...
                        >
                          {sessData.worked_days} day
                          {sessData.worked_days !== 1 ? "s" : ""} worked ·{" "}
                          {sessData.closed_sessions} session
                          {sessData.closed_sessions !== 1 ? "s" : ""} closed
                        </div>
                      )}
                          ...
                          {sessData.daily_summary.map((d, i) => (
                            <div
                              ...
                              style={{
                                ...
                                padding: "2px 0",
                                borderTop:
                                  i > 0 ? "1px solid #e0f2fe" : undefined,
                              }}
                            >
                              <span>
                                Day {i + 1} — {dayjs(d.date).format("DD MMM")}
                              </span>
                              <span
                ...
                <TextArea
                  ...
                  placeholder={
                    pendingAction.action === "open"
                      ? "Starting Day 1 / resuming work…"
                      : pendingAction.action === "on_hold"
                        ? "Done for today, resuming tomorrow…"
      ...
      <Modal
        ...
        destroyOnClose
        styles={{
          body: { maxHeight: "82vh", overflowY: "auto", padding: "16px 20px" },
        }}
      >
        {designJob && (
          <div>
            ...
            >
              ...
              {designSessData && (
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
                  {designSessData.worked_days || 0} day
                  {(designSessData.worked_days || 0) !== 1 ? "s" : ""}·{" "}
                  {designSessData.closed_sessions || 0} session
                  {(designSessData.closed_sessions || 0) !== 1 ? "s" : ""}{" "}
                  closed
                </div>
              )}
            </div>
            ...
            {(designJob.cart_items || []).some((i) => i.design_file) && (
              <>
                ...
                >
                  ...
                  Reference Design(s) from Order
                </div>
                {(designJob.cart_items || [])
                  .filter((i) => i.design_file)
                  .map((item, idx) => (
                    <DesignFilePreview
                      key={idx}
            ...
            <div style={{ marginBottom: 6 }}>
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                >
                  After selecting your file above, use the uploader below to
                  save the sample to the server:
                </div>
                <UploadHelper
                ...
                onClick={async () => {
                  try {
                    const res = await fetch(
                      `${BASE}/${designJob._id}/upload_design`,
                      {
                        method: "POST",
                        headers: jsonHeader(),
                        body: JSON.stringify({
            ...
            {hasExistingDesign && (
              <>
                ...
                >
                  ✅ A design file has been uploaded. You can approve or reject
                  it below.
                  {designJob.design_status === "rejected" &&
                    designJob.design_rejection_reason && (
                      <div
                        style={{
                          marginTop: 6,
                          color: "#ef4444",
