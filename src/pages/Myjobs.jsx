...
const numberToWords = (num) => {
  ...
  const toW = (n) => {
    ...
    if (n < 100000)
      return toW(Math.floor(n / 1000)) + "Thousand " + toW(n % 1000);
    if (n < 10000000)
      return toW(Math.floor(n / 100000)) + "Lakh " + toW(n % 100000);
...
const buildInvoicePDF = async (job) => {
  ...
  const deliveryChg = job.free_delivery
    ? 0
  ...
  doc.text(`Validity: ${validDate}`, PW - MARGIN - 4, y + 5, {
    align: "right",
  });
  ...
  const totalQty = cartItems.reduce(
    (s, it) => s + parseFloat(it.quantity || 1),
    0,
  );
  doc.setFontSize(7.5);
  doc.setFont("helvetica", "normal");
...
const buildJobSheetPDF = async (job) => {
  ...
  // ── Helper: build a readable size string for any item ──────────────────────
  // Priority order:
  //   1. width × height (with unit) + sq_ft if sq.ft item
  //   2. size field (pre-formatted string)
  //   3. "—"
  const buildSizeString = (it) => {
    const w = parseFloat(it.width || 0);
    const h = parseFloat(it.height || 0);
    const sqft = parseFloat(it.sq_ft || 0);
    const unit = (it.size_unit || "ft").trim();
    const isSqFt = it.quantity_type === "sq.ft" || sqft > 0;

    if (w > 0 && h > 0) {
      // Has explicit dimensions — always show them
      if (isSqFt && sqft > 0) {
        return `${w} × ${h} ${unit}\n(${sqft} ft²)`;
      }
      return `${w} × ${h} ${unit}`;
  ...
  const displayName =
    job.company_name && job.company_name.trim()
      ...
      : job.customer_name || "—";
  const custDetail = [displayName, job.customer_phone]
    .filter(Boolean)
    .join("   |   ");
  ...
  txt(
    estDate || "—",
    MARGIN + doc.getTextWidth("Est. Delivery on :") + 2,
    addressEndY + 6,
  );

  // ── FIX: single y advancement after address block — removed the duplicate increment ──
  y = addressEndY + 14;
  hr(y, LGRAY);
  y += 4;

  // ── FIX: isSqFtItem defined once, used in tableRows below ─────────────────
  const isSqFtItem = (it) =>
    it.quantity_type === "sq.ft" || parseFloat(it.sq_ft) > 0;
  ...
  const tableRows = cartItems.map((it, i) => {
    ...
    // ── FIX: always build size from dimensions first ─────────────────────────
    const size = buildSizeString(it);

    // ── Quantity display ─────────────────────────────────────────────────────
    const sqft = parseFloat(it.sq_ft || 0);
    const qty = isSqFtItem(it) && sqft > 0
      ? `${it.quantity} pcs\n${sqft} ft²`
      : String(it.quantity || 1);

    // ── Line total ───────────────────────────────────────────────────────────
    const lineTotal = isSqFtItem(it)
      ? (parseFloat(it.quantity) || 0) * sqft * (parseFloat(it.price) || 0)
  ...
  const filename = `Job_sheet_${job.job_no || "job"}.pdf`;
  const blob = doc.output("blob");
  return { blob, filename };
};
...
const SectionDivider = ({ icon, title }) => (
  ...
  >
    ...
    <div
      style={{ flex: 1, height: 1, background: THEME.border, marginLeft: 4 }}
    />
  </div>
);
...
const PdfShareModal = ({ open, onClose, job, type = "jobsheet" }) => {
  ...
  const handleWhatsApp = () => {
    ...
    const total = parseFloat(job.total_amount || 0).toLocaleString("en-IN", {
  ...
  const handleNativeShare = async () => {
    if (!pdfReady || !blobRef) {
      message.warning("PDF is still generating…");
      return;
    }
    try {
      ...
      if (
        ...
        await navigator.share({
          title: `${titleLabel} – ${job?.job_no || ""}`,
          files: [file],
        });
      } else {
        handleDownload();
        message.info(
          "Direct sharing not supported on this browser — PDF downloaded instead.",
        );
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: generating ? "#f8fafc" : bgReady,
          border: `1px solid ${generating ? "#e5e7eb" : borderReady}`,
          borderRadius: 12,
          padding: "14px 16px",
          marginBottom: 18,
          display: "flex",
        ...
        {generating ? (
          ...
          <>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div
                style={{ fontWeight: 700, fontSize: 13, color: accentColor }}
              >
                PDF Ready
              </div>
              <div
      ...
      {job && (
        ...
        >
          ...
          <div style={{ textAlign: "right" }}>
            <div
              style={{ fontWeight: 800, fontSize: 15, color: THEME.primary }}
            >
              ₹
      ...
      <div style={{ marginBottom: 6 }}>
        ...
        >
          {shareOptions.map((opt) => (
            ...
            >
              <span style={{ color: opt.disabled ? THEME.textMuted : "#fff" }}>
                {opt.icon}
              </span>
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 700,
                  color: opt.disabled ? THEME.textMuted : "#fff",
                  textAlign: "center",
                  lineHeight: 1.2,
                }}
              >
      ...
      >
        💡 <strong>Tip:</strong> For WhatsApp & Email, the PDF is downloaded
        automatically so you can attach it manually.
      </div>
    </Modal>
  );
};
...
const JobDetailView = ({
  ...
  isMobile,
  onOpenJobSheetShare,
  onOpenQuotationShare,
  isQuotationAuthorized,
}) => {
  if (!job) return null;
  const addr = job.delivery_address || {};
  const fullAddress = [
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          >
            {job.company_name ? (
              <>
                <ShopOutlined
                  style={{ color: THEME.primary, fontSize: 12, flexShrink: 0 }}
                />
                {job.company_name}
              </>
            ) : (
          ...
          return (
            ...
            >
              ...
              >
                {isSqFt && it.width ? (
                  <span>
                    Size: {it.width} × {it.height} {it.size_unit || ""} (
                    {it.sq_ft} ft²)
                  </span>
                ) : it.width && it.height ? (
                  // ── FIX: show dimensions even in quantity mode ──
                  <span>
                    Size: {it.width} × {it.height} {it.size_unit || "ft"}
                  </span>
                ) : it.size ? (
      ...
      <div style={grid4}>
        {[
          ...
          { label: "Payment Mode", value: job.payment_mode },
          {
            ...
            value: job.payment_amount ? `₹${job.payment_amount}` : null,
          },
          {
            label: "Order Date",
          ...
          ...(job.current_stage?.assigned_to?.name
            ? [
                {
                  label: "Assigned To",
                  value: job.current_stage.assigned_to.name,
                },
              ]
            : []),
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: THEME.textPrimary, marginTop: 2 }}>
          {job.customer_name || "—"}
        </div>
        <div style={{ fontSize: 11, color: THEME.textSecondary }}>
          {job.customer_phone || ""}
  ...
  return (
    <div
      style={{
        ...
        boxShadow: isPending
          ? "0 0 0 2px #fef3c7, 0 3px 10px rgba(0,0,0,0.06)"
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, color: "#111827" }}>
              {req.requested_by?.name || "Unknown"}
            </div>
            <div style={{ fontSize: 11, color: "#6b7280" }}>
              {req.requested_by?.role || "Designer"}
            </div>
          </div>
        ...
        {req.review_notes && (
          <div
            style={{
              ...
              border: `1px solid ${req.status === "rejected" ? "#fca5a5" : "#86efac"}`,
              borderRadius: 8,
              padding: "8px 10px",
              marginBottom: 10,
              fontSize: 11,
              color: req.status === "rejected" ? "#7f1d1d" : "#14532d",
            }}
          >
        ...
        {isPending && (
          <div style={{ display: "flex", gap: 8 }}>
            <Button
              ...
              icon={<CheckCircleOutlined />}
              size="small"
              loading={acting === `approve-${req._id}`}
...
const InfoRequestsPanel = ({ userProfile }) => {
  ...
  }, [infoTab, loadInfoRequests]);

  const pendingCount = requests.filter((r) => r.status === "pending").length;

  const openApprove = (req) => {
    setActionTarget({ req, type: "approve" });
    setReviewNotes("");
    setTtlHours(24);
    setActionModal(true);
  };
  const openReject = (req) => {
    setActionTarget({ req, type: "reject" });
    setReviewNotes("");
    setActionModal(true);
  };
  ...
  const shown =
    infoTab === "all" ? requests : requests.filter((r) => r.status === infoTab);
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 800, fontSize: 14, color: "#111827" }}>
              Customer Info Access Requests
            </div>
            <div style={{ fontSize: 12, color: "#6b7280" }}>
              Review designer requests to view customer contact details
              {pendingCount > 0 && (
                <span
                  style={{ marginLeft: 8, color: "#d97706", fontWeight: 700 }}
                >
        ...
        <Tooltip title="Refresh">
          <Button
            ...
            onClick={() => loadInfoRequests(infoTab)}
              ...
              <Empty
                ...
                description={
                  <span style={{ color: "#9ca3af", fontSize: 13 }}>
                    No {infoTab === "all" ? "" : infoTab} requests
                  </span>
                }
              />
            </div>
      ...
      <Modal
        ...
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {actionTarget?.req && (
              <Tag
                color={actionTarget.type === "approve" ? "green" : "red"}
                style={{
                  fontFamily: "monospace",
                  fontWeight: 700,
                  fontSize: 11,
        ...
        footer={[
          ...
          <Button
            ...
            style={{
              background:
                actionTarget?.type === "approve" ? "#16a34a" : "#ef4444",
              border: "none",
            }}
          >
          ...
          <div>
            ...
            >
              ...
              <div style={{ fontWeight: 700, color: "#111827", fontSize: 14 }}>
                {actionTarget.req.requested_by?.name}
              </div>
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>
                Job: <strong>{actionTarget.req.job_no}</strong>
              </div>
            ...
            {actionTarget.type === "approve" && (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[4, 8, 24, 48].map((h) => (
                    <Button
                      key={h}
                      size="small"
                      type={ttlHours === h ? "primary" : "default"}
                ...
                <div style={{ marginTop: 6, fontSize: 11, color: "#6b7280" }}>
                  Designer will have access for {ttlHours} hour
                  {ttlHours !== 1 ? "s" : ""} after approval.
                </div>
              </div>
            )}
            <label
...
const Myjobs = () => {
  ...
  const pollPendingInfo = useCallback(async () => {
    try {
      const res = await fetch(`${INFO_BASE}?status=pending&limit=1`, {
        headers: authHeader(),
      });
      const d = await res.json();
      if (d.success) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      message.success(
        `Job ${approvingJob.job_no} approved & assigned to ${name}`,
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          <div
            style={{ fontWeight: 600, fontSize: 13, color: THEME.textPrimary }}
          >
            {r.customer_name || "—"}
          </div>
          <div style={{ fontSize: 11, color: THEME.textSecondary }}>
    ...
    <div style={{ padding: p, background: THEME.bgPage, minHeight: "100vh" }}>
        ...
        >
          <div>
            ...
            >
              ...
              jobs · Last refreshed {lastRefreshed.format("HH:mm:ss")}
              {pendingInfoCount > 0 && (
                <span
                  style={{
                    marginLeft: 10,
                ...
                >
                  · <KeyOutlined style={{ marginRight: 3 }} />
                  {pendingInfoCount} info request
                  {pendingInfoCount !== 1 ? "s" : ""} pending
                </span>
              )}
            </p>
          </div>
      ...
      >
        <Tabs
          ...
          onChange={(k) => {
            setMainTab(k);
            if (k === "info-requests") pollPendingInfo();
          }}
          items={mainTabItems}
      ...
      <Modal
        ...
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontWeight: 700, color: THEME.danger }}>
              Delete Job
            </span>
          </div>
        }
        ...
        {deletingJob && (
          <div>
            ...
            >
              ...
              <div>
                <div
                  style={{ fontWeight: 700, color: THEME.danger, fontSize: 13 }}
                >
            ...
            <div>
              ...
              {deleteError && (
                <div
                  style={{ color: THEME.danger, fontSize: 12, marginTop: 6 }}
                >
                  ⚠ {deleteError}
                </div>
              )}
              {deleteNotes.trim().length > 0 &&
                deleteNotes.trim().length < 50 && (
                  <div
                    style={{ color: THEME.warning, fontSize: 12, marginTop: 6 }}
                  >
