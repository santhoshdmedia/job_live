...
const getOverdueInfo = (estimated_delivery_date) => {
  ...
  return {
    diff,
    label: diff === 1 ? "Overdue 1d" : `Overdue ${diff}d`,
    badge: `+${diff}`,
    color: "#991b1b",
    bg: "#fee2e2",
    border: "#fca5a5",
...
const OverdueBadge = ({ estimated_delivery_date, jobStatus }) => {
  ...
  return (
    <Tooltip
      title={`${info.label} · Est: ${dayjs(estimated_delivery_date).format("DD MMM YYYY")}`}
    >
      <span
        style={{
          display: "inline-flex",
...
const PaymentDueBadge = ({ next_due_date, balance_amount }) => {
  if (!next_due_date || !balance_amount || parseFloat(balance_amount) <= 0)
    return null;
  const today = dayjs().startOf("day");
  ...
  if (diff < 0) {
    ...
    bg = "#fee2e2";
    border = "#fca5a5";
    label = `Payment overdue ${Math.abs(diff)}d`;
  } else if (diff === 0) {
    color = "#92400e";
    bg = "#fef3c7";
    border = "#fcd34d";
    ...
    bg = "#fff7ed";
    border = "#fdba74";
    label = `Payment due in ${diff}d`;
  } else {
    color = "#1e40af";
    bg = "#eff6ff";
    border = "#93c5fd";
    label = `Payment due ${dayjs(next_due_date).format("DD MMM")}`;
  }
  return (
    <Tooltip
      title={`Balance ₹${parseFloat(balance_amount).toFixed(2)} · Due ${dayjs(next_due_date).format("DD MMM YYYY")}`}
    >
      <span
        style={{
          display: "inline-flex",
...
const getItemRequestLine = (item) => {
  ...
  if (cat === "service_office") {
    const qtyLabel =
      item.office_type === "website"
        ? `${item.days || 0} day(s)`
        : item.office_type === "social_media"
          ? `${item.reels_count || 0} reels + ${item.post_count || 0} posts`
          : `${item.hours || 0} hr(s)`;
    return {
      name: item.service_name || item.office_type || "Office Work",
      qty: qtyLabel,
      size: "",
      notes: item.notes || "",
    };
  ...
  if (cat === "service_labour") {
    ...
    return {
      name: item.service_name || "Labour Work",
      qty: parts.join(" + ") || "—",
      size: "",
      notes: item.notes || "",
    };
...
const generateJobRequestImage = (job) => {
  ...
  ctx.save();
  roundRect(ctx, cardX, cardX, cardW, HEADER_H, {
    tl: 16,
    tr: 16,
    br: 0,
    bl: 0,
...
const generateJobRequestPDF = (job) => {
  ...
  items.forEach((item, idx) => {
    ...
    if (notes) {
      const nl = doc.splitTextToSize(`   Notes: ${notes}`, 500);
...
const computeOfficeServiceLineTotal = (item) => {
  ...
  else if (item.office_type === "social_media")
    qty = (item.reels_count || 0) + (item.post_count || 0);
...
const calcJobTotals = (job) => {
  ...
  if (!discountAmt && job.discount_percentage)
    discountAmt = subtotal * ((parseFloat(job.discount_percentage) || 0) / 100);
  discountAmt = Math.min(discountAmt, subtotal);
  const taxableAmount = subtotal - discountAmt;
  const discountRatio = subtotal > 0 ? discountAmt / subtotal : 0;
  const adjustedTaxAmount = parseFloat(
    (taxAmount * (1 - discountRatio)).toFixed(2),
  );
  ...
  return {
    ...
    deliveryCharges,
    grandTotal:
      taxableAmount + adjustedTaxAmount + designCharges + deliveryCharges,
...
const extractTotal = (d, fb) =>
  typeof d?.data?.pagination?.total === "number"
...
const FormField = ({ label, required, children, hint }) => (
  <div>
    ...
    {children}
    {hint && (
      <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 3 }}>{hint}</div>
    )}
  </div>
);
...
const SiteVisitPhotosPanel = ({ photos }) => {
  ...
  return (
    <div
      style={{
        background: "linear-gradient(135deg,#f5f3ff,#ede9fe22)",
    ...
    >
      <SectionHeader
        icon={<CameraOutlined style={{ color: "#7c3aed" }} />}
        title={`Site Visit Photos (${photos.length})`}
      />
      <Image.PreviewGroup>
        <div
          style={{
          ...
          {photos.map((photo, i) => (
            <div key={photo._id || i}>
              ...
              {photo.taken_at && (
                ...
                >
                  {dayjs(photo.taken_at).format("DD MMM, HH:mm")}
                </div>
              )}
            </div>
          ))}
...
const PaymentInfoPanel = ({ job, onCollectPayment }) => {
  ...
  if (hasDue) {
    ...
    if (diff < 0) {
      ...
      dueBg = "#fee2e2";
      dueBorder = "#fca5a5";
      dueMsg = `Overdue by ${Math.abs(diff)} day${Math.abs(diff) > 1 ? "s" : ""}`;
    } else if (diff === 0) {
      dueColor = "#92400e";
      dueBg = "#fef3c7";
      dueBorder = "#fcd34d";
      ...
      dueBg = "#fff7ed";
      dueBorder = "#fdba74";
      dueMsg = `Due in ${diff} day${diff > 1 ? "s" : ""}`;
    } else {
      dueMsg = `Due in ${diff} days`;
    }
  }
  return (
    <div
    ...
    >
      ...
      <div
        style={{
          ...
          gap: 10,
          marginBottom: hasDue ? 12 : 0,
        }}
      >
        <InfoRow
          label="Total Paid"
          value={paid > 0 ? `₹${paid.toFixed(2)}` : "Unpaid"}
          valueStyle={{ color: paid > 0 ? "#16a34a" : "#dc2626" }}
        />
        {balance > 0 && (
          <InfoRow
            label="Balance Due"
        ...
        {hasDue && (
          <InfoRow
            label="Next Due Date"
            value={dayjs(nextDue).format("DD MMM YYYY")}
            valueStyle={{ color: dueColor, fontWeight: 700 }}
          />
        )}
      </div>
      {hasDue && (
        ...
        >
          ...
          />
          <div>
            <div style={{ fontWeight: 700, color: dueColor, fontSize: 13 }}>
              ₹{balance.toFixed(2)} balance due on{" "}
              {dayjs(nextDue).format("dddd, DD MMM YYYY")}
            </div>
            <div
              style={{
                fontSize: 11,
          ...
          >
            {[...history].reverse().map((p, i) => (
              ...
              >
                  ...
                  <div style={{ fontWeight: 700, color: "#1a1a2e" }}>
                    ...
                    {p.discount_applied > 0 && (
                      <span style={{ fontWeight: 400, color: "#059669" }}>
                        {" "}
                        · Disc: ₹{parseFloat(p.discount_applied).toFixed(2)}
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 10, color: "#9ca3af" }}>
                    {p.paid_at
                      ? dayjs(p.paid_at).format("DD MMM YYYY, HH:mm")
                      : ""}
                    {p.notes ? ` · ${p.notes}` : ""}
                  </div>
                </div>
                ...
                >
                  Bal after: ₹{parseFloat(p.balance_after ?? 0).toFixed(2)}
                </div>
              </div>
            ))}
          </div>
...
const ProductItemRow = ({
  ...
  const sizeChange = (field, val) => {
    ...
    if (!updated.sq_ft_manual)
      updated.sq_ft = parseFloat(
        toSqFt(updated.width, updated.height, updated.size_unit).toFixed(4),
      );
  ...
  const handleQtyTypeChange = (val) =>
    onChange(idx, {
      ...
      quantity_type: val,
      ...(val === "quantity"
        ? { sq_ft: 0, sq_ft_manual: false, width: "", height: "" }
        : {}),
    });
  ...
  return (
    ...
    >
        ...
        >
          <FormField label="Width" required={isSqFtMode}>
            <Input
              ...
              prefix={
                <span
                  style={{ fontSize: 10, color: "#6b7280", fontWeight: 700 }}
                >
                  W
                </span>
              }
          ...
          <FormField label="Sq. Ft">
            <InputNumber
              ...
              onChange={(v) =>
                onChange(idx, {
                  ...item,
                  sq_ft: parseFloat((v || 0).toFixed(4)),
                  sq_ft_manual: true,
                })
              }
            />
      ...
      >
        ...
        <FormField
          label={isSqFtMode ? "Price / sq.ft (₹)" : "Unit Price (₹)"}
          required
        >
          <InputNumber
            min={0}
      ...
      <div style={{ marginBottom: 10 }}>
        <FormField label="Design File">
          <DesignFileUpload
            ...
            onChange={(path) => set("design_file", path)}
          />
        </FormField>
      </div>
      <div
...
const OfficeServiceItemRow = ({
  ...
  const set = (f, v) => onChange(idx, { ...item, [f]: v });
  const selectedType = OFFICE_WORK_TYPES.find(
    (t) => t.value === item.office_type,
  );
  const {
    base,
    gstAmt,
    total: lineTotal,
    qty,
  } = computeOfficeServiceLineTotal(item);
      ...
      return (
        ...
        >
          <FormField label="Reels Count">
            <InputNumber
              min={0}
              value={item.reels_count}
              size="small"
              style={{ width: "100%", borderRadius: 6 }}
              addonAfter="reels"
              onChange={(v) => set("reels_count", v || 0)}
            />
          </FormField>
    ...
    >
      ...
      >
        <div
          style={{
            gridColumn:
              item.office_type === "social_media" ? "1/-1" : undefined,
          }}
        >
          {renderCalcFields()}
        </div>
      ...
      >
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: "#92400e" }}>
          Service Total: ₹{lineTotal.toFixed(2)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Popconfirm
          title="Remove this labour entry?"
          onConfirm={() => onRemove(idx)}
          disabled={isOnly}
          okText="Yes"
          cancelText="No"
      ...
      <div style={{ marginBottom: 10 }}>
        <FormField label="Work Description">
          <Input
            placeholder="e.g. Flex installation, cutting, finishing…"
            value={item.service_name}
            size="small"
            style={{ borderRadius: 6 }}
            onChange={(e) => set("service_name", e.target.value)}
      ...
      >
        <FormField label="Sq. Ft">
          <InputNumber
            min={0}
            step={0.5}
            value={item.sq_ft || undefined}
            placeholder="0"
            size="small"
            style={{ width: "100%", borderRadius: 6 }}
            addonAfter="ft²"
            onChange={(v) => set("sq_ft", v || 0)}
          />
        </FormField>
        <FormField label="Price / Sq. Ft (₹)">
          <InputNumber
            min={0}
            value={item.price_per_sqft || undefined}
            placeholder="0"
            size="small"
            style={{ width: "100%", borderRadius: 6 }}
            prefix="₹"
            onChange={(v) => set("price_per_sqft", v || 0)}
          />
        </FormField>
        ...
        <FormField label="Price / Hour (₹)">
          <InputNumber
            min={0}
            value={item.price_per_hour || undefined}
            placeholder="0"
            size="small"
            style={{ width: "100%", borderRadius: 6 }}
            prefix="₹"
            onChange={(v) => set("price_per_hour", v || 0)}
          />
        </FormField>
      </div>
      <div
      ...
      >
        <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>
          {item.sq_ft > 0 && (
            <span>
              {item.sq_ft} ft² × ₹{item.price_per_sqft} = ₹{sqFtAmt.toFixed(2)}
            </span>
          )}
          {item.sq_ft > 0 && item.hours > 0 && (
            <span style={{ margin: "0 6px", color: "#9ca3af" }}>+</span>
          )}
          {item.hours > 0 && (
            <span>
              {item.hours} hrs × ₹{item.price_per_hour} = ₹{hoursAmt.toFixed(2)}
            </span>
          )}
        </div>
        <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: "#6b21a8" }}>
          Labour Total: ₹{lineTotal.toFixed(2)}
        </div>
      </div>
    </div>
  );
    ...
    },
    {
      ...
      label: (
        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {officeItems.length + labourItems.length > 0 && (
            <Tag
              color="orange"
              style={{
                margin: 0,
            ...
            >
              {officeItems.length + labourItems.length}
            </Tag>
          )}
        </span>
      ),
...
const AdminJobManagement = () => {
  ...
  // ── Start / End date range filter (replaces the old single-date filter) ────
  // Value shape: [dayjs, dayjs] | null. Matches against order_date OR
  // estimated_delivery_date, inclusive of both endpoints.
  ...
  const getExportQtyDisplay = (item) => {
    ...
    if (cat === "service_office") {
      ...
      if (item.office_type === "social_media")
        return `${item.reels_count || 0} reels + ${item.post_count || 0} posts`;
    ...
    if (item.quantity_type === "sq.ft") {
      const sizeStr =
        item.width && item.height
          ...
          : "";
      return [`${item.quantity || 0} qty`, `${item.sq_ft || 0} sq.ft`, sizeStr]
        .filter(Boolean)
        .join(" · ");
    }
  ...
  const exportJobsToExcel = (jobsToExport) => {
    ...
    jobsToExport.forEach((job, jobIdx) => {
      const items =
        job.cart_items && job.cart_items.length ? job.cart_items : [{}];
      const globalSerial = jobIdx + 1;
      items.forEach((item, idx) => {
        const isFirstRow = idx === 0;
        const itemAmount =
          item.line_total !== undefined ? parseFloat(item.line_total || 0) : "";
        rows.push({
  ...
  const exportJobsToTallyFormat = (jobsToExport) => {
    ...
    jobsToExport.forEach((job) => {
      ...
      const voucherDate = job.order_date
        ? dayjs(job.order_date).format("DD-MM-YYYY")
        : job.createdAt
      ...
      pushRow(
        ...
        "",
        "Sales",
        `Being sale to ${job.customer_name || "customer"} vide Job ${voucherNo}`,
      );
      ...
      if (totals.taxableAmount > 0) {
        pushRow("Sales A/c", "", parseFloat(totals.taxableAmount.toFixed(2)));
        addLedger("Sales A/c", 0, totals.taxableAmount);
      }
      // Credit: Output CGST / SGST
      if (cgst > 0) {
        pushRow("Output CGST A/c", "", cgst);
        addLedger("Output CGST A/c", 0, cgst);
      }
      if (sgst > 0) {
        pushRow("Output SGST A/c", "", sgst);
        addLedger("Output SGST A/c", 0, sgst);
      }
      // Credit: Design / Delivery charges
      if (totals.designCharges > 0) {
        pushRow(
          "Design Charges A/c",
          "",
          parseFloat(totals.designCharges.toFixed(2)),
        );
        addLedger("Design Charges A/c", 0, totals.designCharges);
      }
      if (totals.deliveryCharges > 0) {
        pushRow(
          "Delivery Charges A/c",
          "",
          parseFloat(totals.deliveryCharges.toFixed(2)),
        );
        addLedger("Delivery Charges A/c", 0, totals.deliveryCharges);
      }
      // Debit: Discount Allowed (contra to party receivable)
      if (totals.discountAmt > 0) {
        pushRow(
          "Discount Allowed A/c",
          parseFloat(totals.discountAmt.toFixed(2)),
          "",
        );
        addLedger("Discount Allowed A/c", totals.discountAmt, 0);
      }
    ...
    const totalDebit = parseFloat(
      dayBookRows
        .reduce((a, r) => a + (typeof r.Debit === "number" ? r.Debit : 0), 0)
        .toFixed(2),
    );
    const totalCredit = parseFloat(
      dayBookRows
        .reduce((a, r) => a + (typeof r.Credit === "number" ? r.Credit : 0), 0)
        .toFixed(2),
    );
    dayBookRows.push({
      "Sr.No": "",
    ...
    const ledgerTotalDr = ledgerRows.reduce(
      (a, r) => a + (typeof r.Debit === "number" ? r.Debit : 0),
      0,
    );
    const ledgerTotalCr = ledgerRows.reduce(
      (a, r) => a + (typeof r.Credit === "number" ? r.Credit : 0),
      0,
    );
    ledgerRows.push({
      "Ledger Name": "TOTAL",
      Debit: parseFloat(ledgerTotalDr.toFixed(2)),
      Credit: parseFloat(ledgerTotalCr.toFixed(2)),
    });
    ...
    XLSX.writeFile(
      wb,
      `Tally-DayBook-${dayjs().format("YYYY-MM-DD_HHmm")}.xlsx`,
    );
  };
  ...
  const startAutoRefresh = useCallback(() => {
    ...
    countdownRef.current = setInterval(
      () =>
        setCountdown((p) => (p <= 1 ? AUTO_REFRESH_INTERVAL / 1000 : p - 1)),
      1000,
    );
  ...
  const filteredJobs = useMemo(() => {
    ...
    // Start/End date range filter — matches if EITHER the order date or the
    // estimated delivery date falls within the [start, end] window (inclusive).
    if (searchDateRange && searchDateRange[0] && searchDateRange[1]) {
      const rangeStart = searchDateRange[0].startOf("day");
      const rangeEnd = searchDateRange[1].endOf("day");
      const inRange = (d) => {
        if (!d) return false;
        const dd = dayjs(d);
        return (
          (dd.isAfter(rangeStart) || dd.isSame(rangeStart)) &&
          (dd.isBefore(rangeEnd) || dd.isSame(rangeEnd))
        );
      };
      result = result.filter((j) => inRange(j.order_date));
    }
    if (statusFilter)
      result = result.filter((j) => j.job_status === statusFilter);
    // Payment Pending filter: balance > 0
    if (paymentPending)
      result = result.filter((j) => parseFloat(j.balance_amount || 0) > 0);
    return result;
  }, [
    jobs,
    searchName,
  ...
  const paymentPendingCount = useMemo(
    () => jobs.filter((j) => parseFloat(j.balance_amount || 0) > 0).length,
    [jobs],
  );
  ...
  const hasActiveFilters =
    searchName ||
    searchJobNo ||
    (searchDateRange && searchDateRange[0] && searchDateRange[1]) ||
    statusFilter ||
    paymentPending;
  ...
  const editTotals = useMemo(() => {
    let subtotal = 0,
      taxAmount = 0;
    editProductItems.forEach((it) => {
      const r = computeProductLineTotal(it);
      subtotal += r.base;
      taxAmount += r.gstAmt;
    });
    editOfficeItems.forEach((it) => {
      const r = computeOfficeServiceLineTotal(it);
      subtotal += r.base;
      taxAmount += r.gstAmt;
    });
    editLabourItems.forEach((it) => {
      const r = computeLabourLineTotal(it);
      subtotal += r.base;
      taxAmount += r.gstAmt;
    });
  ...
  const getLiveBalance = useCallback(
    (job) => {
      if (editJob && job._id === editJob._id) {
        ...
        return Math.max(
          0,
          parseFloat((editTotals.grandTotal - alreadyPaid).toFixed(2)),
        );
      }
    ...
    try {
      ...
      const body = isCustomer
        ? {
            ...
            approved_by_admin_id: profile._id || null,
        ...
        : {
            ...
            assign_to: {
              ...
              name:
                selectedDesigner.name || selectedDesigner.fullName || "Unknown",
            },
          };
      ...
      SUCCESS_NOTIFICATION({
        message: isCustomer
          ? `Job ${approvingJob.job_no} approved (customer design)`
          : `Job ${approvingJob.job_no} approved & assigned to ${selectedDesigner.name || "designer"}`,
      });
  ...
  const handleCollectPayment = async () => {
    ...
    const balance = parseFloat(
      payingJob?.live_balance ?? payingJob?.balance_amount ?? 0,
    );
    const discount = parseFloat(paymentForm.discount_amount) || 0;
    const amt = parseFloat(paymentForm.amount);
    if (!amt || amt <= 0) {
      setCollectPaymentError("Enter a valid payment amount.");
      return;
    }
    if (discount < 0) {
      setCollectPaymentError("Discount cannot be negative.");
      return;
    }
    if (discount > balance + 0.01) {
      setCollectPaymentError(
        `Discount (₹${discount.toFixed(2)}) cannot exceed the balance (₹${balance.toFixed(2)}).`,
      );
      return;
    }
    const effectiveBalance = Math.max(0, balance - discount);
    if (amt > effectiveBalance + 0.01) {
      setCollectPaymentError(
        `Amount cannot exceed the balance after discount (₹${effectiveBalance.toFixed(2)}).`,
      );
      return;
    }
    ...
    try {
      ...
      const body = {
        ...
        next_due_date:
          remaining > 0 && paymentForm.next_due_date
            ? dayjs(paymentForm.next_due_date).toISOString()
            : null,
      };
      const res = await fetch(
        `https://api.dmedia.in/api/jobs/${payingJob._id}/collect-payment`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      SUCCESS_NOTIFICATION({
        message:
          `₹${amt.toFixed(2)} payment recorded for ${payingJob.job_no}` +
          (discount > 0 ? ` (₹${discount.toFixed(2)} discount applied)` : ""),
      });
      ...
      setCollectPaymentError(err.message || "Failed to record payment");
    } finally {
  ...
  const openEditModal = (record) => {
    ...
    if (!storedDisc && legacyPct > 0) {
      const sub = (record.cart_items || []).reduce(
        (acc, it) => acc + computeProductLineTotal(it).base,
        0,
      );
    ...
    setEditProductItems(
      mappedProducts.length ? mappedProducts : [{ ...EMPTY_PRODUCT_ITEM }],
    );
    const offices = allItems.filter(
      (it) => it.item_category === "service_office",
    );
    setEditOfficeItems(
      offices.map((it) => ({
        ...EMPTY_OFFICE_ITEM,
    ...
    const labours = allItems.filter(
      (it) => it.item_category === "service_labour",
    );
    setEditLabourItems(
      labours.map((it) => ({
        ...EMPTY_LABOUR_ITEM,
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const validOffice = editOfficeItems.filter(
        (it) => it.office_type && (it.price || 0) > 0,
      );
      const validLabour = editLabourItems.filter(
        (it) => it.sq_ft > 0 || it.hours > 0,
      );
      ...
      const cartItems = [
        ...
        ...validOffice.map((it) => {
          const { base, gstAmt, total, qty } =
            computeOfficeServiceLineTotal(it);
          return {
            item_category: "service_office",
            product_name: it.service_name || it.office_type,
            office_type: it.office_type,
            days: it.days || 0,
            hours: it.hours || 0,
            reels_count: it.reels_count || 0,
            post_count: it.post_count || 0,
            quantity: qty,
            quantity_type:
              it.office_type === "website"
                ? "days"
                : it.office_type === "social_media"
                  ? "items"
                  : "hours",
            price: it.price,
            gst_percentage: it.gst_percentage || 0,
        ...
        ...validLabour.map((it) => {
          ...
          return {
            item_category: "service_labour",
            product_name: it.service_name || "Labour Work",
            sq_ft: it.sq_ft || 0,
            hours: it.hours || 0,
            price_per_sqft: it.price_per_sqft || 0,
            price_per_hour: it.price_per_hour || 0,
            quantity: 1,
            quantity_type: "labour",
            price: base || 0,
            gst_percentage: it.gst_percentage || 0,
            gst_amount: parseFloat(gstAmt.toFixed(2)),
            line_base: parseFloat(base.toFixed(2)),
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (n, record) => {
        ...
        const hasPaymentDue =
          record.next_due_date && parseFloat(record.balance_amount || 0) > 0;
          ...
          >
            ...
            <Tag
              ...
              style={{
                ...
                fontSize: 11,
                margin: 0,
                border: isSV ? "1px solid #a78bfa" : undefined,
              }}
            >
              {n || "—"}
            </Tag>
        ...
        return (
          <div>
            ...
            {isSV && r.site_visit_no && (
              ...
              >
                <CompassOutlined style={{ fontSize: 9 }} /> {r.site_visit_no}
              </div>
            )}
            {isMobile &&
              (() => {
      ...
      : []),
    {
      ...
      render: (_, r) => {
        const balance = parseFloat(r.balance_amount || 0),
          paid = parseFloat(r.payment_amount || 0);
        return (
          <div>
            <div
              style={{
            ...
            >
              ₹
              {parseFloat(r.total_amount || 0).toLocaleString("en-IN", {
      ...
      ? [
          {
            ...
            render: (_, r) => {
              const stage = r.current_stage?.stage,
                label = WORKFLOW_STAGES.find((s) => s.value === stage)?.label;
              return stage ? (
                <Tag
                  color="purple"
      ...
      : []),
    {
      ...
      render: (_, r) => {
        if (!r.approved_by && !r.approved_by_admin_id)
          return <span style={{ color: "#9ca3af", fontSize: 12 }}>—</span>;
        return (
          <Tooltip
            title={
              r.approved_by_admin_id
                ? `Admin ID: ${r.approved_by_admin_id}`
                : ""
            }
          >
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          ...
          <Tooltip title="Download Job Request as Image">
            <Button
              ...
              onClick={() => generateJobRequestImage(record)}
            >
              {!isMobile && "Job Request"}
            </Button>
          </Tooltip>
          {parseFloat(record.balance_amount || 0) > 0 && (
            <Tooltip title="Collect Payment">
              <Button
                ...
                onClick={() => openCollectPaymentModal(record)}
              >
                {!isMobile && "Collect"}
              </Button>
            </Tooltip>
    ...
    <div style={{ padding: p, background: "#f8fafc", minHeight: "100vh" }}>
        ...
        >
          <div>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>
              <strong>{filteredJobs.length}</strong> job
              {filteredJobs.length !== 1 ? "s" : ""}
              {hasActiveFilters ? " (filtered)" : ` · Total: ${jobs.length}`} ·
              Refreshed {lastRefreshed.format("HH:mm:ss")}
            </p>
          </div>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            ...
            <FormField label="Filter by Date Range (Order Date)">
              <RangePicker
          ...
          {hasActiveFilters && (
            ...
            >
              ...
              {searchDateRange && searchDateRange[0] && searchDateRange[1] && (
                <Tag
                  closable
                  onClose={() => setSearchDateRange(null)}
                  color="orange"
                  style={{ fontSize: 11 }}
                >
                  Order Date: {searchDateRange[0].format("DD MMM YYYY")} →{" "}
                  {searchDateRange[1].format("DD MMM YYYY")}
                </Tag>
              )}
              {statusFilter && (
                <Tag
                  ...
                  color={STATUS_CONFIG[statusFilter]?.color || "default"}
                  style={{ fontSize: 11 }}
                >
              ...
              {paymentPending && (
                <Tag
                  ...
                  style={{ fontSize: 11 }}
                >
                  💰 Payment Pending ({filteredJobs.length})
                </Tag>
              )}
              <Button
                size="small"
                ...
                style={{ padding: "0 4px", fontSize: 11, color: "#6b7280" }}
              >
                Clear all
              </Button>
            </div>
        ...
        {paymentPending && filteredJobs.length > 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 12, color: "#b45309" }}>
              · Total outstanding: ₹
              {filteredJobs
                .reduce((acc, j) => acc + parseFloat(j.balance_amount || 0), 0)
          ...
          >
            ...
            {viewJob?.estimated_delivery_date && (
              <OverdueBadge
                ...
                jobStatus={viewJob.job_status}
              />
            )}
            {viewJob?.next_due_date &&
              parseFloat(viewJob.balance_amount || 0) > 0 && (
                <PaymentDueBadge
                  next_due_date={viewJob.next_due_date}
                  balance_amount={viewJob.balance_amount}
                />
              )}
            {viewJob && isFullyCompletedAndPaid(viewJob) && (
              ...
              >
                <CheckCircleOutlined style={{ fontSize: 10 }} /> Completed &
                Paid
              </span>
            )}
          </div>
            ...
            return (
              ...
              >
                {completedAndPaid && (
                  ...
                  >
                    <CheckCircleOutlined
                      style={{ color: "#065f46", fontSize: 22, flexShrink: 0 }}
                    />
                    <div>
                      <div
                        style={{
                      ...
                      >
                        ✓ Job Completed & Fully Paid
                      </div>
                      <div
                        style={{ fontSize: 11, color: "#047857", marginTop: 2 }}
                      >
                        No balance remaining. This job is closed.
                      </div>
                    </div>
                  </div>
                )}
                {isFromSV && (
                  <div
                    style={{
                      ...
                      borderRadius: 10,
                      background:
                        "linear-gradient(135deg,#f5f3ff 0%,#ede9fe55 100%)",
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        Created from a Site Visit
                      </div>
                      {viewJob.site_visit_no && (
                ...
                {isFromSV && viewJob.site_visit_photos?.length > 0 && (
                  <SiteVisitPhotosPanel photos={viewJob.site_visit_photos} />
                )}
                {!completedAndPaid &&
                  overdueInfo &&
                  (overdueInfo.isOverdue || overdueInfo.isDueToday) && (
                    <div
                      style={{
                        display: "flex",
                        alignItems: "center",
                ...
                >
                  ...
                  >
                    ...
                    {viewJob.company_name && (
                      <InfoRow label="Company" value={viewJob.company_name} />
                    )}
                    <InfoRow
                      label="Est. Delivery"
                      value={
                    ...
                    <InfoRow
                      ...
                      value={
                        viewJob.createdAt
                          ? dayjs(viewJob.createdAt).format(
                              "DD MMM YYYY, HH:mm",
                            )
                          : "—"
                      }
                ...
                <div>
                  <SectionHeader
                    ...
                    title={`Job Items (${(viewJob.cart_items || []).length})`}
                  />
                  <ViewCartItems
                    cartItems={viewJob.cart_items}
                    isMobile={isMobile}
                  />
                </div>
                <div
                ...
                <PaymentInfoPanel
                  job={viewJob}
                  onCollectPayment={() => openCollectPaymentModal(viewJob)}
                />
                {viewJob.notes && (
                  <div
                    style={{
      ...
      >
        <Spin spinning={editLoading}>
          {editJob && isSiteVisitJob(editJob) && (
            <div
              style={{
                ...
                padding: "12px 16px",
                borderRadius: 10,
                background: "linear-gradient(135deg,#f5f3ff,#ede9fe44)",
                border: "2px solid #a78bfa",
                display: "flex",
                gap: 12,
                alignItems: "flex-start",
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  Site Visit Job — fill in all details
                </div>
                {editJob.site_visit_no && (
                  <span
                    style={{
                      fontSize: 11,
                      color: "#6d28d9",
                ...
                {editJob.site_visit_photos?.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    >
                      Site Visit Photos — Reference for sizing
                    </div>
                    <Image.PreviewGroup>
                      <div
          ...
          <JobItemsSection
            productItems={editProductItems}
            officeItems={editOfficeItems}
            labourItems={editLabourItems}
            onProductChange={(i, u) =>
              setEditProductItems((p) => p.map((it, j) => (j === i ? u : it)))
            }
            onOfficeChange={(i, u) =>
              setEditOfficeItems((p) => p.map((it, j) => (j === i ? u : it)))
            }
            onLabourChange={(i, u) =>
              setEditLabourItems((p) => p.map((it, j) => (j === i ? u : it)))
            }
            onAddProduct={() =>
              setEditProductItems((p) => [...p, { ...EMPTY_PRODUCT_ITEM }])
            }
            onAddOffice={() =>
              setEditOfficeItems((p) => [...p, { ...EMPTY_OFFICE_ITEM }])
            }
            onAddLabour={() =>
              setEditLabourItems((p) => [...p, { ...EMPTY_LABOUR_ITEM }])
            }
            onRemoveProduct={(i) =>
              setEditProductItems((p) => p.filter((_, j) => j !== i))
            }
            onRemoveOffice={(i) =>
              setEditOfficeItems((p) => p.filter((_, j) => j !== i))
            }
            onRemoveLabour={(i) =>
              setEditLabourItems((p) => p.filter((_, j) => j !== i))
            }
            isMobile={isMobile}
            isTablet={isTablet}
          />
          ...
          >
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <InfoRow
                label="Paid So Far"
                value={`₹${parseFloat(editJob?.payment_amount || 0).toFixed(2)}`}
                valueStyle={{ color: "#16a34a" }}
              />
              <InfoRow
                label="Current Balance"
                value={`₹${parseFloat(editJob?.balance_amount || 0).toFixed(2)}`}
                valueStyle={{
                  color:
                    parseFloat(editJob?.balance_amount || 0) > 0
                      ? "#dc2626"
                      : "#16a34a",
                }}
              />
              {editJob?.next_due_date &&
                parseFloat(editJob?.balance_amount || 0) > 0 && (
                  <InfoRow
                    label="Next Due"
                    value={dayjs(editJob.next_due_date).format("DD MMM YYYY")}
                  />
                )}
            </div>
            <Button
              icon={<WalletOutlined />}
              onClick={() => openCollectPaymentModal(editJob)}
              disabled={!(parseFloat(editJob?.balance_amount || 0) > 0)}
              style={{
                borderRadius: 8,
                color: "#d97706",
                borderColor: "#fde68a",
          ...
          <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 14 }}>
            Payments are recorded separately and won't be affected by item/price
            changes here — the balance will recalculate after you save.
          </div>
          ...
          >
            ...
            {hasRounding && (
              <SummaryRow
                label={
                  editTotals.roundingAdj > 0 ? "Rounding (+)" : "Rounding (−)"
                }
              ...
              <>
                ...
                >
                  ...
                  <span
                    style={{
                      color:
                        projectedBalanceAfterSave <= 0 ? "#059669" : "#dc2626",
                      background:
                        projectedBalanceAfterSave <= 0 ? "#f0fdf4" : "#fef2f2",
                      padding: "2px 10px",
                      borderRadius: 6,
                    }}
                  >
        ...
        {approvingJob && (
          <div>
            ...
            >
              ...
              {isSiteVisitJob(approvingJob) && (
                ...
                >
                  ...
                  >
                    Site Visit:{" "}
                    {approvingJob.site_visit_no || approvingJob.site_visit_id}
                  </span>
                </div>
              )}
              {(() => {
                const cfg =
                  STATUS_CONFIG[approvingJob.job_status] || STATUS_CONFIG.draft;
                return (
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              >
                Assign Designer <span style={{ color: "#ef4444" }}>*</span>
              </label>
              <Select
                placeholder={
              ...
              {selectedDesigner?._id === "customer_designed" && (
                ...
                >
                  ...
                  <span>
                    No internal designer assigned — customer-provided design.
                  </span>
                </div>
              )}
              {!designersLoading &&
      ...
      <Modal
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontWeight: 700 }}>Collect Payment</span>
          </div>
        }
        open={collectPaymentModal}
        onCancel={closeCollectPaymentModal}
        maskClosable={!collectingPayment}
        closable={!collectingPayment}
        footer={[
          <Button
            key="cancel"
              ...
              <div>
                ...
                >
                  ...
                  >
                    <span style={{ fontSize: 12, color: "#6b7280" }}>
                      Total:{" "}
                      <strong>
                        ₹{parseFloat(payingJob.total_amount || 0).toFixed(2)}
                      </strong>
                    </span>
                    <span
                      style={{
                  ...
                  {payingJob.live_balance !== undefined &&
                    Math.abs(
                      payingJob.live_balance -
                        parseFloat(payingJob.balance_amount || 0),
                    ) > 0.01 && (
                      <div
                        style={{
                          marginTop: 8,
                      ...
                      >
                        ⚠ Balance reflects unsaved item changes. Save the job
                        first to lock in this amount.
                      </div>
                    )}
                </div>
                {collectPaymentError && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <TagOutlined style={{ color: "#d97706" }} /> Discount /
                      Waiver (Optional)
                    </div>
                    <FormField
                      label="Discount Amount (₹)"
                      hint="Amount to waive off from the balance before collecting payment"
                    >
                      <InputNumber
                        min={0}
                        max={balance}
                  ...
                  <FormField label="Amount Received (₹)" required>
                    ...
                    >
                      ...
                      >
                        Full Balance
                        {discount > 0 ? ` (₹${effectiveBal.toFixed(2)})` : ""}
                      </button>
                      <button
                        onClick={() =>
                          setPaymentForm((p) => ({
                            ...p,
                            amount: parseFloat((effectiveBal / 2).toFixed(2)),
                          }))
                        }
                        style={{
                          fontSize: 11,
                  ...
                  <FormField label="Payment Mode">
                    <Select
                      ...
                      value={paymentForm.method || undefined}
                      style={{ width: "100%" }}
                      allowClear
                      onChange={(v) =>
                        setPaymentForm((p) => ({ ...p, method: v || "" }))
                      }
                    >
                      {PAYMENT_MODES.map((m) => (
                  ...
                  {remaining > 0 && amt > 0 && (
                    <FormField
                      label="Next Due Date for Remaining Balance"
                      hint={`₹${remaining.toFixed(2)} will still be due`}
                    >
                      <DatePicker
                        value={
                          paymentForm.next_due_date
                            ? dayjs(paymentForm.next_due_date)
                            : null
                        }
                        onChange={(d) =>
                          setPaymentForm((p) => ({ ...p, next_due_date: d }))
                        }
                        format="DD MMM YYYY"
                        style={{ width: "100%", borderRadius: 8 }}
                        disabledDate={(d) =>
                        ...
                        {[
                          ...
                          <button
                            ...
                            onClick={() =>
                              setPaymentForm((p) => ({
                                ...p,
                                next_due_date: dayjs().add(days, "day"),
                              }))
                            }
                            style={{
                              fontSize: 11,
