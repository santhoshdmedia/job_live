...
// ─── UnitQtyInput ─────────────────────────────────────────────────────────────
const UnitQtyInput = ({ value, onChange, disabled = false, size = "large" }) => {
  const qty = value?.qty ?? "";
  return (
    <InputNumber
      value={qty === "" ? null : qty}
        ...
        <Tooltip key={unit} title={`IN: ${formatQty(total_in, unit)} | OUT: ${formatQty(total_out, unit)}`}>
          <div className="flex justify-between gap-2 text-xs cursor-help">
            <span className="font-semibold px-1 rounded"
              style={{ background: unit === primaryUnit ? "#eff6ff" : "#f3f4f6", color: unit === primaryUnit ? "#1d4ed8" : "#6b7280", fontSize: 10 }}>
              {unitLabel(unit)}
            </span>
...
const StockRangeFilter = ({ stockMin, stockMax, onStockMinChange, onStockMaxChange, onClear }) => {
  ...
  const isPresetActive = (p) => stockMin === (p.min ?? "") && stockMax === (p.max ?? "");
        ...
        {STOCK_PRESETS.map((preset) => {
          ...
          return (
            <button key={preset.label} type="button" onClick={() => handlePreset(preset)}
              style={{ background: active ? preset.color : preset.bg, borderColor: active ? preset.color : preset.border, color: active ? "#fff" : preset.color, border: "1.5px solid", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.15s", outline: "none" }}>
              {preset.label}
            </button>
          );
        })}
        ...
        {hasActive && (
          <Tooltip title="Clear stock range">
            <Button size="large" onClick={onClear}
              style={{ borderRadius: 10, background: "#f3f4f6", border: "1px solid #e5e7eb", color: "#6b7280", flexShrink: 0, fontWeight: 700 }}>✕</Button>
          </Tooltip>
        )}
      </div>
    </div>
...
const PriceRangeFilter = ({ priceMin, priceMax, onPriceMinChange, onPriceMaxChange, onClear }) => {
  ...
  const isPresetActive = (p) => priceMin === (p.min ?? "") && priceMax === (p.max ?? "");
...
const MaterialIssueHistoryModal = ({ open, onClose, product }) => {
  ...
  const columns = [
    ...
    { title: "Issued To",  dataIndex: "issued_to",  key: "issued_to",  render: (v) => <span className="text-xs">{v || "—"}</span> },
    { title: "Issued By",  dataIndex: "issued_by",  key: "issued_by",  render: (v) => <span className="text-xs">{v || "—"}</span> },
    ...
    { title: "Notes", dataIndex: "notes", key: "notes", render: (v) => <span className="text-xs text-gray-500">{v || "—"}</span> },
  ];
  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} width={1050} destroyOnClose
      ...
      <Table dataSource={issues.map((item, i) => ({ ...item, key: `issue-${i}` }))}
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Form.Item label="Product Type" name="type">
              <Select placeholder="Select type" className="h-10" options={[
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setProductSize((prev) => ({ ...prev, width: e.target.value }))}
                ...
                <select
                  ...
                  className="px-3 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-yellow-400"
                >
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setProductSize((prev) => ({ ...prev, height: e.target.value }))}
                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400"
                  placeholder="Height"
                />
...
const StockInModal = ({ open, onClose, product, onSuccess }) => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const result = await editProduct({ stock_info: updatedStockInfo, unit_stock_summary: newSummary, stock_count: legacyStockCount }, product._id);
      SUCCESS_NOTIFICATION(result);
      onSuccess();
      onClose();
    } catch (err) {
    ...
    <Modal open={open} onCancel={onClose} footer={null} width={700} destroyOnClose
      title={
        <div className="flex items-center gap-3 pb-2 border-b border-gray-100">
          ...
          <div className="ml-auto flex gap-2 flex-wrap">
            <Tag color="blue"    className="font-semibold text-xs">Primary unit: {UNIT_MAP[primaryUnit]?.icon} {unitLabel(primaryUnit)}</Tag>
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit} className="mt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            {unitQty.qty > 0 && (
              <div className="mt-2 px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2"
                ...
                <ArrowUpOutlined /> Adding {formatQty(unitQty.qty, primaryUnit)} to stock
              </div>
            )}
          </div>
          ...
          <Form.Item label="Date & Time" name="date" rules={[formValidation("Select a date")]}>
            <DatePicker showTime className="h-10 w-full" format="DD/MM/YYYY h:mm A" placeholder="Select date & time" />
          </Form.Item>
          <Form.Item label="Invoice No." name="invoice">
            <Input placeholder="INV-001" className="h-10" />
          </Form.Item>
          <Form.Item label="Invoice Date & Time" name="invoice_date">
            <DatePicker showTime className="h-10 w-full" format="DD/MM/YYYY h:mm A" placeholder="Select invoice date & time" />
          </Form.Item>
          <Form.Item label="Notes" name="notes" className="md:col-span-2">
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const existingStockOffline = _.get(product, "stock_offline", []).map((item) => ({
        ...item, date: item.date ? new Date(item.date).toISOString() : null,
      }));
      const newEntry = {
        stock:            qty,
        unit_qty:         { qty, unit: primaryUnit },
      ...
      const result = await editProduct({ stock_offline: updatedStockOffline, unit_stock_summary: newSummary, stock_count: Math.max(0, legacyCount) }, product._id);
      SUCCESS_NOTIFICATION(result);
      onSuccess();
      onClose();
    } catch (err) {
      ...
      title={
        <div className="flex items-center gap-3 pb-2 border-b border-gray-100">
          ...
          <div className="ml-auto flex gap-2 flex-wrap">
            ...
            <Tag color="error" className="font-semibold">Current: {formatQty(currentStock, primaryUnit)}</Tag>
          </div>
        </div>
      }>
      <Form form={form} layout="vertical" onFinish={handleSubmit} className="mt-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <UnitQtyInput value={unitQty}
              onChange={(v) => { if (v.qty > currentStock) message.warning(`Max available: ${formatQty(currentStock, primaryUnit)}`); setUnitQty(v); }} />
            {unitQty.qty > 0 && (
              <div className="mt-2 px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2"
                style={{ background: "#fff1f2", border: "1px solid #fecaca", color: "#dc2626" }}>
                <ArrowDownOutlined /> Removing {formatQty(unitQty.qty, primaryUnit)} from stock
              </div>
            )}
          </div>
          <Form.Item label="Handler Name" name="handler_name" rules={[formValidation("Enter handler name")]}>
...
const StockHistoryModal = ({ open, onClose, product }) => {
  ...
  const inColumns = [
    { title: "Date & Time",   dataIndex: "date",         key: "date",         width: 145, render: (t) => <span className="text-xs text-gray-600">{t}</span> },
    { title: "Qty",           dataIndex: "unit_qty",     key: "qty",          width: 110, align: "center", render: (uq) => <span className="font-bold text-green-600 text-sm">+{formatQty(uq.qty, uq.unit)}</span> },
    { title: "Handler",       dataIndex: "handler_name", key: "handler_name", render: (t) => <span className="text-xs">{t}</span> },
    { title: "Location",      dataIndex: "location",     key: "location",     render: (t) => <span className="text-xs">{t}</span> },
    { title: "Invoice",       dataIndex: "invoice",      key: "invoice",      render: (t) => <span className="text-xs">{t}</span> },
    { title: "Invoice Date",  dataIndex: "invoice_date", key: "invoice_date", width: 145, render: (t) => <span className="text-xs text-gray-600">{t}</span> },
    { title: "Notes",         dataIndex: "notes",        key: "notes",        render: (t) => <span className="text-xs">{t}</span> },
    { title: "Images", dataIndex: "images", key: "images", width: 100,
      render: (imgs) => imgs?.length > 0 ? (
        <div className="flex gap-1 flex-wrap">
          {imgs.slice(0, 3).map((img, i) => <Image key={i} src={img.url || img.path || img} width={32} height={32} className="object-cover rounded" preview />)}
  ...
  const outColumns = [
    { title: "Date & Time",   dataIndex: "date",             key: "date",             width: 145, render: (t) => <span className="text-xs text-gray-600">{t}</span> },
    { title: "Qty",           dataIndex: "unit_qty",         key: "qty",              width: 110, align: "center", render: (uq) => <span className="font-bold text-red-600 text-sm">−{formatQty(uq.qty, uq.unit)}</span> },
    { title: "Handler",       dataIndex: "handler_name",     key: "handler_name",     render: (t) => <span className="text-xs">{t}</span> },
    { title: "Location",      dataIndex: "location",         key: "location",         render: (t) => <span className="text-xs">{t}</span> },
    { title: "Customer",      dataIndex: "customer_details", key: "customer_details", render: (t) => <span className="text-xs">{t}</span> },
    { title: "Notes",         dataIndex: "notes",            key: "notes",            render: (t) => <span className="text-xs">{t}</span> },
  ];
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-gray-100">
          ...
          <div className="ml-auto flex gap-2 flex-wrap">
            {allUnits.map((u) => {
              ...
              return <Tag key={u} color="blue" className="font-semibold text-xs">{UNIT_MAP[u]?.icon} Net {unitLabel(u)}: {netStock.toLocaleString()}</Tag>;
            })}
          </div>
        </div>
      }>
...
const renderSizeCell = (size) => {
  ...
  const wLabel = `${size.width ?? "—"} ${wUnit}`;
  const hLabel = `${size.height ?? "—"} ${hUnit}`;
  return (
...
const AddProduct = () => {
  ...
  const handlePageChange    = (currentPage) => { setPaginationConfig((p) => ({ ...p, currentPage })); savePaginationToStorage(paginationConfig.pageSize, currentPage); };
  const handlePageSizeChange = (pageSize)   => { setPaginationConfig((p) => ({ ...p, pageSize, currentPage: 1 })); savePaginationToStorage(pageSize, 1); };
  const handleTabChange     = (key)         => { setActiveTabKey(key); localStorage.setItem(STORAGE_KEYS.ACTIVE_TAB, key); handlePageChange(1); };

  useEffect(() => { fetchCategories(); fetchMainCategories(); collectVendors(); }, []);
  useEffect(() => { fetchData(); }, [search, filterByProductCategory, filterByType, filterByProductSubcategory, vendorFilter, visibilityFilter]);

  const fetchData = async () => {
    try {
      setLoading(true);
      const result = await getProduct("", search || "", true, filterByProductCategory || "", filterByType || "", filterByProductSubcategory || "", vendorFilter || "", visibilityFilter || "");
  ...
  const fetchMainCategories = async () => {
    try { const result = await getAllCategoryProducts(); setMainCategoryData(_.get(result, "data.data", [])); }
    catch (err) { ERROR_NOTIFICATION(err); }
  };

  const collectVendors = async () => {
    try { setLoading(true); const result = await getAllVendor(); setAllVendors(_.get(result, "data.data", [])); }
    catch (err) { ERROR_NOTIFICATION(err); }
    finally { setLoading(false); }
  };
  ...
  const handleDeleteProduct = (record) => {
    confirm({
      ...
      content: <div><p className="text-gray-700 mb-1">Are you sure you want to delete <strong>{record.name}</strong>?</p><p className="text-xs text-red-500">This action cannot be undone.</p></div>,
  ...
  const getTotalStock = (product) => product.stock_count || 0;
  ...
  const handleClearFilters = () => {
    ...
    setStockMin(""); setStockMax(""); setPriceMin(""); setPriceMax(""); handlePageChange(1);
  ...
  const stockFilteredData = useMemo(() => {
    ...
    return getCurrentTabData.filter((item) => {
      ...
      return (stockMin === "" || stock >= Number(stockMin)) && (stockMax === "" || stock <= Number(stockMax));
    });
  ...
  const processedTableData = useMemo(() =>
    priceFilteredData.map((item, index) => ({ ...item, serialNumber: index + 1, totalStock: getTotalStock(item) })),
    [priceFilteredData]);
  ...
  const columns = [
      ...
      render: (_, record) => {
        ...
        return (
          <div className="flex justify-center">
            {img ? (
              <div className="relative w-16 h-16 rounded-xl overflow-hidden border border-gray-200 bg-white shadow">
    ...
    { title: "Material Brand", key: "material_brand", dataIndex: "material_brand", width: 140,
      render: (brand) => brand ? (
        <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-semibold" style={{ background: "#eef2ff", color: "#4338ca", border: "1px solid #c7d2fe" }}>
          🏷️ {brand}
        </span>
        ...
        <div className="flex items-center justify-center gap-1.5 flex-wrap">
          <Tooltip title="Stock IN">
            <Button size="small" icon={<ArrowUpOutlined />} onClick={() => setStockInProduct(record)}
              style={{ background: "#f0fdf4", borderColor: "#16a34a", color: "#16a34a", fontWeight: 600, fontSize: 11, borderRadius: 8 }}>IN</Button>
          </Tooltip>
          <Tooltip title="Stock OUT">
            <Button size="small" icon={<ArrowDownOutlined />} onClick={() => setStockOutProduct(record)}
              style={{ background: "#fff1f2", borderColor: "#dc2626", color: "#dc2626", fontWeight: 600, fontSize: 11, borderRadius: 8 }}>OUT</Button>
          </Tooltip>
  ...
  const paginationProps = {
    current: paginationConfig.currentPage, pageSize: paginationConfig.pageSize,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-50 p-4 md:p-8 font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-1">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5 m-0">Manage inventory, stock movements &amp; products</p>
            </div>
            <Input placeholder="Search products..." value={search} onChange={(e) => setSearch(e.target.value)}
              allowClear className="h-9 rounded-xl border-gray-200 sm:w-64"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Button icon={<MdFileDownload />} onClick={exportToCSV} loading={exportLoading}
              ...
              style={{ background: "#f0fdf4", borderColor: "#16a34a", color: "#16a34a" }}>Export CSV</Button>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => setNewProductModal(true)}
              className="rounded-xl font-bold h-9 px-5 shadow-md"
              style={{ background: "#0d9488", borderColor: "#0d9488", fontSize: 13 }}>New Product</Button>
          </div>
        </div>
      </div>
          ...
          <Title level={5} className="m-0 flex items-center gap-2 text-gray-900">
            ...
            {activeFilterCount > 0 && (
              <span className="ml-1 inline-flex items-center justify-center w-5 h-5 rounded-full text-white font-bold"
                style={{ background: "#0d9488", fontSize: 11 }}>{activeFilterCount}</span>
            )}
          </Title>
          <div className="flex gap-2">
        ...
        <div className={`transition-all duration-500 overflow-hidden ${showFilters ? "max-h-[900px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <Select placeholder="Select Type" size="large" className="w-full" allowClear onChange={(val) => setFilterByType(val)} value={filterByType}
                options={[{ value: "Stand Alone Product", label: "Stand Alone Product" }, { value: "Variable Product", label: "Variable Product" }, { value: "Variant Product", label: "Variant Product" }]} />
            </div>
            <div>
              <Text className="text-sm font-semibold text-gray-700 mb-1 block">Visibility</Text>
              <Select placeholder="Visibility" size="large" className="w-full" allowClear onChange={(val) => setVisibilityFilter(val)} value={visibilityFilter}
                options={[{ value: "", label: "All Products" }, { value: "true", label: "Visible Only" }, { value: "false", label: "Hidden Only" }]} />
            </div>
          </div>
          <div className="rounded-2xl p-4 mb-4" style={{ background: "linear-gradient(135deg, #f0fdf4 0%, #eff6ff 100%)", border: "1.5px solid #bbf7d0" }}>
            <div className="flex items-center gap-2 mb-3">
          ...
          <div className="rounded-2xl p-4" style={{ background: "linear-gradient(135deg, #f5f3ff 0%, #eff6ff 100%)", border: "1.5px solid #ddd6fe" }}>
            <div className="flex items-center gap-2 mb-3">
      ...
      <Card className="bg-white shadow-xl rounded-3xl border-none" bodyStyle={{ padding: 0 }}>
        <Tabs destroyInactiveTabPane type="card" size="large" className="px-4 md:px-8 pt-4"
          activeKey={activeTabKey} onChange={handleTabChange}
          items={[{
            key: "1",
            label: (
              <span className="flex items-center font-bold text-gray-900 text-sm">
                📦 Products
                <Tag className="ml-2 bg-teal-100 text-teal-800 font-semibold rounded-full px-2 text-xs">{processedTableData.length}</Tag>
              </span>
            ),
            children: (
      ...
      {/* ── Modals ── */}
      <NewProductStockModal open={newProductModal} onClose={() => setNewProductModal(false)} onSuccess={fetchData} />
      {stockInProduct  && <StockInModal  open={true} product={stockInProduct}  onClose={() => setStockInProduct(null)}  onSuccess={() => { fetchData(); setStockInProduct(null);  }} />}
