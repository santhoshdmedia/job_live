  ...
  return (
    <div>
      ...
      {!busy && origSize && compSize && (
        ...
        >
          ✅ {fmt(origSize)} → {fmt(compSize)}
          {compSize <= 500 ? " ✓" : " ⚠ Still large"}
        </div>
      )}
      {busy && (
        <div
      ...
      {error && (
        <div style={{ marginTop: 4, fontSize: 10, color: T.danger }}>
          ⚠ {error}
        </div>
      )}
      {previewUrl && !busy && (
...
const SectionHeader = ({ icon, title, count }) => (
  ...
  >
    <span style={{ color: T.primary, fontSize: 14, flexShrink: 0 }}>
      {icon}
    </span>
    <span
      style={{
    ...
    <div
      style={{ flex: 1, height: "1.5px", background: T.border, marginLeft: 4 }}
    />
  </div>
);
...
const InfoBadge = ({ label, value, color, sub }) => (
  <div style={{ flex: 1, minWidth: 80 }}>
    ...
    {sub && (
      <div style={{ fontFamily: T.font, fontSize: 9, color: T.mutedFg }}>
        {sub}
      </div>
    )}
  </div>
    ...
    >
      ...
      >
        ...
        <FormField label="Material">
          <Select
            ...
            size="small"
            style={{ width: "100%", fontFamily: T.font }}
            disabled={!selected}
        ...
        >
          <FormField label="Width" required>
            <Input
              ...
              prefix={
                <span
                  style={{ fontSize: 10, color: T.mutedFg, fontWeight: 700 }}
                >
                  W
                </span>
              }
          ...
          <FormField
            label={
              <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                {!item.sq_ft_manual && (
                  <span style={{ fontSize: 9, color: T.mutedFg }}>(auto)</span>
                )}
              </span>
            }
          >
            <InputNumber
              ...
              style={{
                width: "100%",
                borderRadius: T.radius,
                borderColor: item.sq_ft_manual ? T.primary : undefined,
                background:
                  item.sq_ft > 0
                    ? item.sq_ft_manual
                      ? T.primaryLt
      ...
      >
        <FormField label="Quantity" required>
          <InputNumber
            ...
            value={item.quantity}
            size="small"
            style={{ width: "100%", borderRadius: T.radius }}
      ...
      >
        ...
        >
          ...
          {item.gst_percentage > 0 && (
            <span style={{ marginLeft: 8 }}>
              ...
              <span style={{ fontWeight: 600, color: T.warning }}>
                ₹{gstAmt.toFixed(2)}
              </span>
            </span>
          )}
        ...
        {isSqFtMode && item.sq_ft > 0 && (
          <div style={{ fontFamily: T.font, fontSize: 10, color: T.success }}>
            {item.quantity} × {item.sq_ft} ft² × ₹{item.price}
          </div>
        )}
        {!isSqFtMode && (
          <div style={{ fontFamily: T.font, fontSize: 10, color: T.mutedFg }}>
      ...
      >
        ...
        >
          ...
          {item.sq_ft > 0 && item.hours > 0 && (
            <span style={{ margin: "0 6px", color: T.mutedFg }}>+</span>
          )}
          {item.hours > 0 && (
            <span>
...
const CreateJobModal = ({ open, onClose, onCreated }) => {
  ...
  const addProduct = () =>
    setProductItems((p) => [...p, { ...EMPTY_PRODUCT_ITEM }]);
  const addOffice = () =>
    setOfficeItems((p) => [...p, { ...EMPTY_OFFICE_ITEM }]);
  const addLabour = () =>
    setLabourItems((p) => [...p, { ...EMPTY_LABOUR_ITEM }]);

  const removeProduct = (i) =>
    setProductItems((p) => p.filter((_, j) => j !== i));
  const removeOffice = (i) =>
    setOfficeItems((p) => p.filter((_, j) => j !== i));
  const removeLabour = (i) =>
    setLabourItems((p) => p.filter((_, j) => j !== i));
  ...
  const calcTotals = useCallback(() => {
    ...
    const del = formData.free_delivery
      ? 0
      : parseFloat(formData.delivery_charges) || 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const validOffice = officeItems.filter(
        (it) => it.office_type && (it.price || 0) > 0,
      );
      const validLabour = labourItems.filter(
      ...
      const cartItems = [
        ...validProducts.map((it) => {
          ...
          return {
            ...
            size:
              it.quantity_type === "sq.ft" && it.width && it.height
      ...
      const payload = {
        ...
        delivery_address: {
          street: [formData.address_line1, formData.address_line2]
            .filter(Boolean)
            .join(", "),
          city: formData.city,
        ...
        delivery_charges: formData.free_delivery
          ? 0
          : parseFloat(formData.delivery_charges) || 0,
            ...
            ? [
                {
                  ...
                  notes: "",
                  next_due_date:
                    t.balance > 0 && formData.next_due_date
  ...
  return (
    <Modal
      ...
      styles={{
        body: {
          maxHeight: isMobile ? "calc(100dvh - 56px)" : "86vh",
          overflowY: "auto",
          padding: isMobile ? 10 : 18,
        },
        header: {
          padding: `${isMobile ? 10 : 14}px ${isMobile ? 12 : 18}px`,
    ...
    >
      <Spin
        ...
        indicator={<LoadingOutlined style={{ color: T.primary }} spin />}
      >
        {/* ── Order info banner ───────────────────────────────────────────── */}
        <div
          style={{
        ...
        >
          <InfoBadge
            label="Order Date"
            value={orderDate.format("DD MMM YYYY")}
          />
          <InfoBadge
            label="Valid Until"
            value={validUntil.format("DD MMM YYYY")}
            color={T.success}
            sub="30 days"
          />
          {adminUser?.name && (
            <InfoBadge label="Created By" value={adminUser.name} />
          )}
        </div>
        ...
        >
          <FormField label="Customer Name" required>
            <Input
              prefix={<UserOutlined style={{ color: T.mutedFg }} />}
          ...
          <FormField label="Phone" required>
            <Input
              prefix={<PhoneOutlined style={{ color: T.mutedFg }} />}
          ...
          <FormField label="Est. Delivery Date" required>
            <DatePicker
              ...
              onChange={(dayjsVal) =>
                handleInput("estimated_delivery_date", dayjsVal)
              }
              style={{
                width: "100%",
                borderRadius: T.radius,
        ...
        >
          <FormField label="Company Name">
            <Input
              prefix={<BankOutlined style={{ color: T.mutedFg }} />}
          ...
          <FormField label="GST Number" hint="15-character GSTIN (optional)">
            <Input
              ...
              onChange={(e) =>
                handleInput("gst_no", e.target.value.toUpperCase())
              }
            />
          </FormField>
        </div>
            ...
            >
              <Input
                ...
                style={{
                  borderRadius: T.radius,
                  fontFamily: T.font,
                  borderColor: pincodeFilled ? T.success : undefined,
                }}
                suffix={
                  pincodeLoading ? (
                    <LoadingOutlined
                    ...
                    <CheckCircleOutlined
                      style={{ color: T.success, fontSize: 12 }}
                    />
                  ) : null
                }
              />
        ...
        >
          <FormField label="Payment Mode">
            <Select
              ...
              value={formData.payment_mode || undefined}
              style={{ width: "100%", fontFamily: T.font }}
              allowClear
          ...
          <FormField label="Delivery Charges (₹)">
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <InputNumber
                ...
                value={
                  formData.free_delivery
                    ? 0
                    : formData.delivery_charges || undefined
                }
                disabled={formData.free_delivery}
                style={{ flex: 1, borderRadius: T.radius }}
                prefix="₹"
              ...
              <div
                onClick={() =>
                  handleInput("free_delivery", !formData.free_delivery)
                }
                title={
                  formData.free_delivery
                    ? "Click to disable free delivery"
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontSize: 11,
                    fontWeight: 700,
                    color: formData.free_delivery ? "#fff" : T.mutedFg,
                    whiteSpace: "nowrap",
                  }}
                >
        ...
        >
          ...
          {/* Line rows */}
          <SummaryRow
            label={`Subtotal (before GST & discount)`}
          ...
          <SummaryRow
            ...
            value={
              formData.free_delivery ? "Free 🎉" : `₹${totals.del.toFixed(2)}`
            }
          ...
          <div
            style={{
              ...
              marginTop: 8,
              padding: "10px 14px",
              background: T.goldenLt,
              border: `1.5px solid ${T.golden}`,
              borderRadius: T.radius,
            }}
          >
            <span
